---
name: wayfinder
description: "Plans a huge chunk of work, more than one agent session can hold, as a shared map of decision tickets on your issue tracker, and resolves them one at a time until the way to the destination is clear."
...
- [<closed ticket title>](link): <verdict-first gist, under ~50 words> (confidence: <high|medium|low>, research only)
...
The answer isn't part of the body, it's recorded on resolution (see [Work through the map](#work-through-the-map)). Assets created while resolving a ticket are linked from the `## Answer` comment, not pasted in. No branch naming rule: use whatever link the skill returns. The `## Answer` comment uses this format:
...
Two modes: chart a loose idea into a map, or work an existing map. Work resolves at most one non-research ticket per session, or up to three research tickets in parallel.
...
2. **Map the frontier.** Grill again, this time fan out across the whole space rather than deep on any one thread, surfacing the open decisions and the first steps takeable now. **If this surfaces no fog and no open decisions**, nothing is left to decide or investigate, the way to the destination is already clear, and you don't need a map. Stop and ask the user how they'd like to proceed.
...
2. Choose tickets. If the user named tickets, use them: one ticket of any type, or up to three research tickets. Otherwise take the first frontier ticket the tracker's frontier query returns. If it is research, you may take up to two more unblocked research tickets to fill a batch of three. **Claim each** with the tracker operation before any work (see [Tickets](#tickets) for claim rules). Treat a claimed ticket as untakeable; if you need a claimed ticket, ask the user first.
3. **Orient.** Re-read the map's Destination and Decisions so far. In one or two sentences, tell the user what this ticket decides and how it advances the frontier toward the destination. Then resolve.
4. Resolve by type, after you **fetch full body of each ticket**. If needed, fetch the full body of any related or closed tickets as well.
...
5. Record each resolution. Post the `## Answer` comment in the ticket's Answer format (see [Tickets](#tickets)). **Close** the ticket. Append a context pointer to the map's Decisions so far, with confidence for research. Keep the map entry minimal. The verdict leads the Answer so it scans. The detail a later session needs sits below it, unless a linked artifact (research file, prototype) already holds it. Carry gaps into fog and follow-up tickets.
6. Add newly-surfaced tickets (Chart step 4's create-then-wire pattern: create first, wire blocking edges second). Graduate any fog the answer has made specifiable. Clear each graduated patch from **Not yet specified** so it lives only as its new ticket. If this ticket or another sits beyond the destination, rule it out of scope. Do not resolve it on the route. If the decision invalidates other parts of the map, close those tickets with the reason in their `## Answer`; never delete them.
7. Close the map when done. If no open tickets remain and **Not yet specified** is empty, the way is clear: close the map and tell the user. If no open tickets remain but fog bullets remain, the map is stuck, not done: leave the map open and report what still blocks graduation.
