...
- `Status:` records each artifact's state. Active implementation tickets use the triage role strings mapped in `docs/agents/triage-labels.md`, and the implementing agent sets `resolved` once the work lands. Wayfinder tickets use `open`/`claimed`/`resolved`; the map uses `open`/`resolved` (see Wayfinding operations below).
- `Blocked by:` lists the numbers of the tickets that must complete first, for example `Blocked by: 01, 03`. A blocker clears when its file is terminal: `resolved`, or the string that `docs/agents/triage-labels.md` maps to `wontfix`.
...
## When a skill says "close the ticket" or "close the map"

Set that artifact's `Status:` line to `resolved`. When a ticket is rejected rather than done, use the string that `docs/agents/triage-labels.md` maps to `wontfix`.
...
- **Map**: `scratch/<effort>/map.md`. The file starts with a `# <map title>` heading and `Status: open`, then the Notes / Decisions-so-far / Fog body. Closing the map sets `Status: resolved`.
- **Child ticket**: `scratch/<effort>/decisions/NN-<slug>.md`, numbered from `01` within `decisions/`, with the question in the body. A `Type:` line records the ticket type (`research`/`prototype`/`grilling`/`task`); a `Status:` line records `open`/`claimed`/`resolved`, and a new ticket starts `open`.
...
- **Blocking**: a `Blocked by: NN, NN` line near the top. A ticket is unblocked when every file it lists is terminal: `resolved`, or the string that `docs/agents/triage-labels.md` maps to `wontfix`.
- **Frontier**: scan `scratch/<effort>/decisions/` for files that are open, unblocked, and unclaimed; first by number wins.
- **Claim**: set `Status: claimed` and save before any work.
- **Release**: set `Status:` back to `open` when a claimed ticket cannot be resolved in this session, so the frontier can pick it up again.
