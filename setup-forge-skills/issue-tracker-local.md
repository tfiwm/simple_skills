...
- `Status:` records each artifact's state. Active implementation tickets use the triage role strings mapped in `docs/agents/triage-labels.md`, and the implementing agent sets `resolved` once the work lands. Wayfinder tickets use `claimed`/`resolved`; the map uses `open`/`resolved` (see Wayfinding operations below).
...
- **Map**: `scratch/<effort>/map.md` (the Notes / Decisions-so-far / Fog body). The file starts with `Status: open`; closing the map sets `Status: resolved`.
