...
- **Issue tracker**: where issues live (GitHub by default; the local markdown tracker is also supported out of the box)
...
- **Local markdown tracker**: issues live as files under `scratch/<feature>/` in this repo (good for solo projects or repos without a remote)
- **Other** (Jira, Linear, etc.): ask the user to describe the workflow in one paragraph; the skill will record it as freeform prose

Record the choice in `docs/agents/issue-tracker.md`. The GitHub and GitLab templates carry a "PRs as a request surface" flag, defaulted **off**. Leave it off and don't raise it: a user who wants external PRs in the triage queue can flip the flag in the file later.

**Section B: Triage label vocabulary.** Always write `docs/agents/triage-labels.md`, even when the `triage` skill isn't installed, because `to-spec` and `to-tickets` read it to apply the `ready-for-agent` label.

Ask the vocabulary question only when `triage` is installed:
